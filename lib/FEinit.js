...
_.copyBuildInTemplate = function(distDir, destDir) {
    ...
    if (gFile.isDir(distDir)) {
        ...
        recurse(distDir, function(abspath, rootdir, subdir, filename, destdir) {
            ...
            if (filename) {
                ...
                if (filename.indexOf('_') === 0) {
                    ...
                    gFile.write(destDir + '/' + destname, content, {
                        encoding: 'utf-8'
                    });
                } else {
...
function recurse(rootdir, callback, subdir, destDir) {
    var abspath = subdir ? path.join(rootdir, subdir) : rootdir;
    fs.readdirSync(abspath).forEach(function(filename) {
        var filepath = path.join(abspath, filename);
        if (fs.statSync(filepath).isDirectory()) {
            grunt.file.mkdir(path.join(destDir, filename));
            recurse(rootdir, callback, unixifyPath(path.join(subdir, filename)), unixifyPath(path.join(destDir, filename)));
        } else {
            callback(unixifyPath(filepath), rootdir, subdir, filename, destDir);
        }
    });
