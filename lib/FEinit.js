...
_.copy = function() {
    ...
    if (config.download) {
        console.log('Download npm packages!'.green);
        spawn.command('cd ' + _.route + ' && npm install', []);
    } else {
        console.log('Copying npm packages!'.green);
    ...
    for (var i in files) {
        if (keys.indexOf(i) !== -1) {
            if (/y/i.test(props[i])) {
                console.log('Add '.green + props[i].bold.yellow + ' files..');
                //如果是需要
...
_.Gruntfile = function(json) {
    ...
    console.log('Success'.green + ': Gruntfile.js Create!');
    _.dir();
    _.copy();
}
/**
...
_.dir = function() {
    ...
    console.log('Create dirs...'.green);
    dirs.forEach(function(v) {
        var dest = join(_.route, './' + v);
        if (!file.isDir(dest)) {
            console.log('* Create folder'.yellow + ':' + dest);
            file.mkdir(dest);
        }
    });
...
_.package = function(json) {
    ...
    console.log('Success'.green + ': package.json Create!');
    evt.emit('package end');
...
_.showLogo = function() {
    ...
    console.log('  |   ' + ' ______  ______   _         _  _   '.rainbow + '   |');
    console.log('  |   ' + '|  ____||  ____| (_)       (_)| |  '.rainbow + '   |');
    console.log('  |   ' + '| |__   | |__     _  _ __   _ | |_ '.rainbow + '   |');
    console.log('  |   ' + '|  __|  |  __|   | ||  _ \\ | || __|'.rainbow + '   |');
    console.log('  |   ' + '| |     | |____  | || | | || || |_ '.rainbow + '   |');
    console.log('  |   ' + '|_|     |______| |_||_| |_||_| \\__|'.rainbow + '   |');
    console.log('  |_________________________________________|');
    console.log('\n');
}